...
1.图片尺寸大，下载慢。非常占用 Android 壳的缓存，约 70-80M。 2.切换 Tab 卡（尤其在长列表间切换的时候） 3.自定义字体占据 30M，字体 swap 时间长 4.长列表越往下滑动越卡。
    ...
    if (item instanceof Array) {
                ...
                <LineWrap>
                    {item.map((item, index) => (
                        <LessonItem
                            data={item}
                            key={item.planId}
                            handleClick={this.props.onLessonItemClick}
                        />
                    ))}
                </LineWrap>
            </div>
        ...
        return (
            <div style={style}>
                <SubjectTitleWrapper>
                    <SubjectTitle>
                        <SubjectTitleText>{item.text}</SubjectTitleText>
                        <SubjectTitleLine />
                    </SubjectTitle>
                    {item.isFirstSubject && <FilterButton />}
                </SubjectTitleWrapper>
            </div>
        );
